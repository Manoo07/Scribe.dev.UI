...
const StudentAttendanceView = ({
  ...
  const [attendanceHistory, setAttendanceHistory] = useState<
    Array<{ date: string; present: boolean }>
  >([]);
  ...
  const getStatusLevel = () => {
    if (stats.percentage >= 90)
      return { color: "green", text: "Excellent Standing" };
    if (stats.percentage >= 75)
      return { color: "green", text: "Good Standing" };
    if (stats.percentage >= 60)
      return { color: "yellow", text: "Needs Improvement" };
    ...
    <div className="bg-gray-800/60 backdrop-blur-sm p-6 rounded-xl border border-gray-700/50 shadow-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-gradient-to-br from-gray-700/90 to-gray-800/90 p-5 rounded-xl transition-all duration-300 hover:shadow-lg hover:scale-[1.02] group border border-gray-600/50">
          ...
          <div className="text-2xl font-bold text-white group-hover:bg-gradient-to-r from-blue-400 to-indigo-500 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300">
            {stats.percentage}%
          </div>
          <div
            className={`text-sm mt-1 font-medium text-${statusInfo.color}-500`}
          >
            {statusInfo.text}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-gray-700/90 to-gray-800/90 p-5 rounded-xl transition-all duration-300 hover:shadow-lg hover:scale-[1.02] group border border-gray-600/50">
          ...
          <div className="text-2xl font-bold text-green-400 group-hover:bg-gradient-to-r from-green-400 to-emerald-500 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300">
            {stats.presentDays}
          </div>
        ...
        <div className="bg-gradient-to-br from-gray-700/90 to-gray-800/90 p-5 rounded-xl transition-all duration-300 hover:shadow-lg hover:scale-[1.02] group border border-gray-600/50">
          ...
          <div className="text-2xl font-bold text-red-400 group-hover:bg-gradient-to-r from-red-400 to-rose-500 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300">
            {stats.absentDays}
          </div>
          ...
          <table className="min-w-full divide-y divide-gray-700">
            ...
            <tbody className="divide-y divide-gray-700/50 bg-gray-800/40">
              {attendanceHistory.slice(0, 5).map((record, index) => (
                <tr
                  key={record.date}
                  className={`hover:bg-gray-700/70 transition-colors ${
