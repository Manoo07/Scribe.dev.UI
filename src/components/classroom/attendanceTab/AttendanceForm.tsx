...
const AttendanceForm = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`Attendance for ${format(parseISO(selectedDate), "MMMM d, yyyy")} has been updated.`);
      onAttendanceUpdated();
    } catch (error) {
  ...
  if (!selectedDate) {
    return (
      <div className="bg-gray-800 rounded-lg p-8 text-gray-300 flex flex-col items-center justify-center min-h-[300px] border border-gray-700 shadow-lg">
        ...
        <p className="text-center text-gray-500 mt-2">Choose a day from the calendar to begin</p>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit}>
        {students.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-8 text-gray-400">
            ...
            <p className="text-center text-lg">No students found in this classroom</p>
          </div>
        ) : (
